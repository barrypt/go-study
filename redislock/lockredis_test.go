...
func TestLockSuccess(t *testing.T) {
	...
	// 设置模拟锁获取成功的行为
	mock.ExpectEval(luaLock, []string{key}, token, lockTime.Seconds()).SetVal("OK")
...
func TestUnlockFail(t *testing.T) {
	...
	// 解锁逻辑
	mock.ExpectEval(luaUnLock, []string{key}, token+"test-2").SetVal(nil) // 模拟解锁失败
...
func TestSpinLockSuccess(t *testing.T) {
	...
	mock.ExpectEval(luaLock, []string{key}, token, lockTime.Seconds()).SetVal("OK")
	mock.ExpectEval(luaLock, []string{key}, token2, lockTime.Seconds()).SetVal("OK")
...
func TestSpinLockTimeout(t *testing.T) {
	...
	mock.ExpectEval(luaLock, []string{key}, token, lockTime.Seconds()).SetVal("OK")
	mock.ExpectEval(luaLock, []string{key}, token2, lockTime.Seconds()).SetVal("nil")
...
func TestRenewSuccess(t *testing.T) {
	...
	mock.ExpectEval(luaLock, []string{key}, token, lockTime.Seconds()).SetVal("OK")
	mock.ExpectEval(luaRenew, []string{key}, token, lockTime.Seconds()).SetVal("OK")
...
func TestWithTimeout(t *testing.T) {
	...
	mock.ExpectEval(luaLock, []string{key}, token, timeout.Seconds()).SetVal("OK")
...
func TestSevLockSuccess(t *testing.T) {
	...
	err := lock.Lock()
	if err != nil {
		log.Println("lock acquisition failed：", err)
		return
	}
	defer lock.UnLock() // unlock

	log.Println("Task run: 开始")
	go func() {
		log.Println("其他线程开始抢夺锁资源")
		lock := New(ctx, redisClient, key)
		err := lock.Lock()
		// 如果加锁成功
		if err == nil {

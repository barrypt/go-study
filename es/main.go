...
func main() {
	...
	cfg := elasticsearch.Config{
		Addresses:               []string{"http://localhost:9200"}, // Elasticsearch 节点的地址
	...
	mapping := &types.TypeMapping{
		...
		Properties: map[string]types.Property{
			"BetaCode": types.TextProperty{
				Index: &a,
			},
			"Name":  types.TextProperty{},
			"Score": types.FloatNumberProperty{},
		},
	}

	indexName := "45666"

	exist, err := typedClient.Indices.Exists(indexName).Do(context.Background())

	if err != nil && !exist {

		res1, _ := typedClient.Indices.Create(indexName).Mappings(mapping).Do(context.Background())
	...
	bulk, _ := typedClient.Core.Create(indexName, "122").Document(Student{
		BetaCode: "122",
		Name:     "qpt",
		Score:    80,
